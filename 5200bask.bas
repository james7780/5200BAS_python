...
DIM SHARED TOKEN$(127), OPTIONS$(18, 1)
DIM SHARED DOSTACK(31), FORSTACK(31), SELSTACK(31, 2), IFSTACK(31)
COMMON SHARED LINENUM, J, Q$, DEBUG, OPTIONS, TITLE, SCRMODE
...
SUB CMD.KEYPAD
  IF TOKEN$(J + 1) = "FIX" THEN
    ...
    CALL PRINTOUT(".WORD", "$02F0;if zero, skip")
    CALL PRINTOUT("DEC", "KEYDB;otherwise decrease by one")
  ELSE
    IF MID$(Q$, J + 2, 1) <> "N" THEN CALL ERROROUT("Type mismatch")
    T = DEC(TOKEN$(J + 1))
